...
enrollments = pd.DataFrame({
    ...
    "course_id": [101, 102, 101, 102, 103, 104, 103, 104],
    "semester": ["Fall 2023"] * 8,
    "grade": ["A", "A", "B", "A", "A", "A", "A", "B"]
})

df_merged = (
    students.merge(courses, how = 'left', on = 'major')
    .merge(enrollments, how = 'left', on = ['course_id','student_id'])
    .groupby('student_id', as_index = False)
    .agg(
        course_count = ('course_id','count'),
        a_grade_count = ('grade', lambda d:(d == 'A').sum())
    )
    .loc[lambda d: d['course_count'] == d['a_grade_count']]

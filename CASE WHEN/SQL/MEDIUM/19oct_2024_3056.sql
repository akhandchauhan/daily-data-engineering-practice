...
-- Write a solution to calculate the percentage of the total time spent on sending and opening
-- snaps for each age group. Precentage should be rounded to 2 decimal places.
...
INSERT INTO Activities (activity_id, user_id, activity_type, time_spent) 
VALUES
    ('7274', '123', 'open', '4.5'),
    ('2425', '123', 'send', '3.5'),
    ('1413', '456', 'send', '5.67'),
...
SELECT 
    a.age_bucket,
    ROUND(SUM(IF(ac.activity_type = 'send', ac.time_spent, 0)) * 100 / SUM(ac.time_spent), 2) AS send_perc,
    ROUND(SUM(IF(ac.activity_type = 'open', ac.time_spent, 0)) * 100 / SUM(ac.time_spent), 2) AS open_perc
FROM Age a

...
WITH prime_item_info AS (
    SELECT 
        item_type,
        COUNT(*) AS prime_item_count,
        SUM(square_footage) AS total_square_ft,
        FLOOR(500000/SUM(square_footage)) AS total_combo,
        FLOOR(500000/SUM(square_footage)) * COUNT(*) AS total_prime_item_combo
    FROM Inventory
...
final_prime_info AS (
    SELECT 
        item_type,
        total_prime_item_combo AS item_count,
        total_combo * total_square_ft AS total_prime_area
    FROM prime_item_info 
),
non_prime_info AS (
    SELECT 
            item_type,
            COUNT(*) AS prime_item_count,
            SUM(square_footage) AS total_square_ft,
            500000 -(SELECT total_prime_area FROM final_prime_info) AS non_prime_area
    FROM Inventory
...
SELECT item_type,
        item_count 
FROM final_prime_info
UNION ALL
SELECT 
        item_type,
        IFNULL(prime_item_count *FLOOR(non_prime_area/total_square_ft),0) AS item_count
FROM non_prime_info
ORDER BY item_count;
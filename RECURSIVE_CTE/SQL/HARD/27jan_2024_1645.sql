...
CREATE TABLE IF NOT EXISTS AcceptedRides (ride_id INT, driver_id INT, ride_distance INT,
 ride_duration INT);
...
WITH RECURSIVE
    ...
    S AS (
        ...
        ON YEAR(d.join_date) < 2020 OR (YEAR(d.join_date) = 2020 AND MONTH(d.join_date) <= month)
    ),
    T AS (
...
FROM
    S AS s
    LEFT JOIN T AS t
    ON s.driver_id = t.driver_id AND s.join_date <= t.requested_at
    AND s.month = MONTH(t.requested_at)
GROUP BY 1;

---------------------------------------------------------------------------------------------------
-- m2
WITH RECURSIVE month_generated_cte AS (
    SELECT 1 AS month,2020 AS year
    UNION ALL
    SELECT month+1, 2020
    FROM month_generated_cte
    WHERE month < 12
),
accepted_ride_info AS (
    SELECT
        MONTH(r.requested_at) AS month,
        COUNT(DISTINCT ar.driver_id) AS accepted_ride
    FROM rides r 
...
accepted_driver_info AS (
    SELECT 
            mgc.month,
            COUNT(*) AS active_drivers
    FROM month_generated_cte mgc 
    LEFT JOIN Drivers d
    ON d.join_date <= LAST_DAY(CONCAT(mgc.year, '-', mgc.month, '-01'))
    GROUP BY 
            mgc.month
)
SELECT 
    adi.month,
    ROUND(
        IFNULL(ari.accepted_ride * 100.0/adi.active_drivers 
        ,0)
    ,2) AS working_percentage
FROM accepted_driver_info adi 
LEFT JOIN accepted_ride_info ari 

...
-- Each row of this table contains the ID of a ride, the user's ID that requested it, and 
-- the day they requested it.
...
-- The average_ride_distance is calculated by summing up the total ride_distance values 
-- from the three months and dividing it by 3. The average_ride_duration is calculated 
-- in a similar way.

-- Return the result table ordered by month in ascending order, where month is the 
-- starting month's number (January is 1, February is 2, etc.).
...
final AS (
    SELECT
            mi.month,
            ROUND(AVG(COALESCE(total_ride_distance,0)) OVER(
                ORDER BY mi.month
                ROWS BETWEEN CURRENT ROW AND 2 FOLLOWING
            ), 2) AS average_ride_distance,
            ROUND(AVG(COALESCE(total_ride_duration, 0)) OVER(
                ORDER BY mi.month
                ROWS BETWEEN CURRENT ROW AND 2 FOLLOWING
            ),2) AS average_ride_duration
    FROM month_info mi 
    LEFT JOIN agg_ride_info ari
    ON mi.month = ari.month
)
SELECT 
    month,

...
# Each row of this table contains the ID of a ride, the user's ID that requested it,
# and the day they requested it.
...
# Write an SQL query to report the percentage of working drivers (working_percentage) for
# each month of 2020 where:
# Note that if the number of available drivers during a month is zero, we consider the 
# working_percentage to be 0.

# Return the result table ordered by month in ascending order, where month is the month's
# number (January is 1, February is 2, etc.). Round working_percentage to the nearest 2 decimal 
# places.
...
# The percentage is 0%.
# By the end of March --> four active drivers (10, 8, 5, 7) and one accepted ride by driver (10).
...
accepted_rides_df = pd.DataFrame({
...
month_df = pd.DataFrame({"year" : 2020, "month" : np.arange(1,13)})

month_df["last_date"] = (
    pd.to_datetime(
        month_df["year"].astype(str) + "-" + month_df["month"].astype(str) + "-01"
    ) + MonthEnd(1)
)

accepted_rides_driver_df = (
    rides_df.merge(accepted_rides_df, on ='ride_id')
    .loc[lambda d: (d['requested_at'] >= '2020-01-01') & (d['requested_at'] < '2021-01-01')]
    .assign(
        month = lambda d: d['requested_at'].dt.month
    )
    .groupby('month', as_index = False)['driver_id']
    .nunique()
    .rename(columns = {'driver_id' :'accepted_rides'})
)

active_driver_df =(
    month_df.merge(drivers_df, how = 'cross')
    .loc[lambda d: d['join_date'] <= d['last_date']]
    .groupby("month", as_index= False)['driver_id']
    .nunique()
    .rename(columns = {'driver_id' :'active_drivers'})
)
final_df = (
    active_driver_df.merge(accepted_rides_driver_df, how ='left', on ='month')
    .assign(
        working_percentage = lambda d: (d['accepted_rides'] *100.0/d['active_drivers']).round(2).fillna(0)
    )
    .sort_values('month')
    [['month','working_percentage']]
)

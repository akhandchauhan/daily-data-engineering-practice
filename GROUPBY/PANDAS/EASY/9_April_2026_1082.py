...
seller_price_df = (
    sales_df.groupby('seller_id')['price']
    .sum()
    .reset_index()
)

max_price = seller_price_df['price'].max()

df = (
    seller_price_df[seller_price_df['price'] == max_price]
    [['seller_id']]
...
seller_price_df = (
    ...
    .assign(
        seller_rank = (
            lambda d : d['price'].rank(method = 'dense',ascending= False)
        )
    )
    .loc[lambda d: d['seller_rank'] == 1]
    [['seller_id']]
    .reset_index(drop = True)
)

print(seller_price_df)
...
seller_price_df = (
    ...
    .sum()
    .reset_index()
    .loc[lambda d: d['price'] == d['price'].max(), ['seller_id']]
)

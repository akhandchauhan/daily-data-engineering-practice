...
--  An imbalanced order is one whose maximum quantity is strictly greater than the 
-- average quantity of every order (including itself).

-- The average quantity of an order is calculated as (total quantity of all products 
-- in the order) / (number of different products in the order). 
...
(1, 1, 12),(1, 2, 10),(1, 3, 15),(2, 1, 8),(2, 4, 4),(2, 5, 6),(3, 3, 5),(3, 4, 18),
(4, 5, 2),(4, 6, 8),(5, 7, 9),(5, 8, 9),(3, 9, 20),(2, 9, 4);
...
SELECT order_id
FROM order_info
WHERE max_quantity > (SELECT MAX(avg_quantity) FROM order_info)
;
...
max_avg AS (
    SELECT MAX(avg_quantity) AS val FROM order_info
)
SELECT oi.order_id
FROM order_info oi
JOIN max_avg ma
ON oi.max_quantity > ma.val;
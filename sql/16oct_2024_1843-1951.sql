...
--- wrong answer
WITH cte as(
SELECT account_id,YEAR(day) AS yr,month(day) as mo,SUM(IF(type='Debitor',-amount,amount)) as curr_balance
FROM Transactions
...
WITH user_transaction_history as (
    SELECT a.account_id,
        a.max_income,
        DATE_FORMAT(day,'%Y-%m-01') AS trans_mth,
        SUM(amount) as credited_amt
    FROM Accounts a 
    JOIN Transactions t 
...
consecutive_mth_info as (
    SELECT account_id,
        trans_mth,
        LEAD(trans_mth) OVER (PARTITION BY account_id ORDER BY trans_mth) AS nxt_mth
    FROM user_transaction_history
    WHERE credited_amt > max_income
)
SELECT DISTINCT account_id
FROM consecutive_mth_info
WHERE TIMESTAMPDIFF(month, trans_mth,nxt_mth) = 1
ORDER BY 1
;

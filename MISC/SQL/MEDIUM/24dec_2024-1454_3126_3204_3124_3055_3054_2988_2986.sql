...
WITH cte as(
    SELECT *, DENSE_RANK() OVER(PARTITION BY type ORDER BY duration DESC) as rnk
    FROM Contacts  c 
    LEFT JOIN Calls c2
    ON c.id = c2.contact_id
)
SELECT first_name, type, 
DATE_FORMAT(SEC_TO_TIME(duration),'%H:%i:%s') AS duration_formatted
FROM cte
WHERE rnk <=3
order by 2,duration DESC,1;
...
FROM (SELECT *, PERCENT_RANK() OVER(PARTITION BY state ORDER BY fraud_score DESC) as rnk
FROM Fraud) a 
WHERE rnk <=0.05;

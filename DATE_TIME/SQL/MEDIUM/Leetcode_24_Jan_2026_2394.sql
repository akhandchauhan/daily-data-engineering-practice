...
WITH employee_info as (
    SELECT 
        employee_id,
        SUM(TIMESTAMPDIFF(minute, in_time, out_time)) AS login_mins
    FROM logs 
...
WHERE e.needed_hours * 60 > IFNULL(login_mins, 0);
...
LEFT JOIN (
    SELECT
        employee_id,
        SUM(CEIL(TIMESTAMPDIFF(SECOND, in_time, out_time) / 60.0)) AS total_minutes
    FROM Logs
    GROUP BY employee_id
) l
ON e.employee_id = l.employee_id
WHERE e.needed_hours * 60 > IFNULL(l.total_minutes, 0);
...
HAVING
    IFNULL(
        SUM(CEIL(TIMESTAMPDIFF(SECOND, l.in_time, l.out_time) / 60.0)),
        0
    ) < e.needed_hours * 60;

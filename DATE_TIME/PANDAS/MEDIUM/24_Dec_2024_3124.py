...
df['call_rank'] = df.groupby('type')['duration']\
                    .rank(method='first', ascending=False)

df = df[df['call_rank'] <= 3]

df = df.sort_values(
    ['type','duration','first_name'],
    ascending=[False, False, False]
)

df['duration_formatted'] = pd.to_timedelta(df['duration'], unit='s')\
                                .astype(str).str[-8:]

df = df[['first_name','type','duration_formatted']]

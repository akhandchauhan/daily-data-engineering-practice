...
--m2 my method is wrong here, since filtering should be done before ranking
WITH team_ranking_info AS (
    SELECT 
            employee_id,
            name,
            salary,
            COUNT(employee_id) OVER(PARTITION BY salary) AS salary_group_count,
            DENSE_RANK() OVER(ORDER BY salary) AS team_id
    FROM Employees 
)
SELECT 
        ...
        salary,
        team_id
FROM team_ranking_info
WHERE salary_group_count > 1
ORDER BY team_id,
          employee_id;
...
-- Each row contains information about a call the time of a call between caller_id 
-- and recipient_id.
-- Write an  SQL query to report the IDs of the user who had the first and the last 
-- call with the same person on any day.
...
-- Similary, User 4 had the first and last call on 2021-08-24 with
-- user 8. User 4 should be included in the answer.
-- On 2021-08-11, user 1 and 5 had a call. The call was the
-- only call for both of them on this day. 
-- Since this call is the first and last call of the day for both 
-- of them, they both should be included in the answer.
drop table Calls;
...
cte2 AS (
    SELECT *,
           FIRST_VALUE(recipient_id) OVER(PARTITION BY caller_id, DATE(call_time) ORDER BY call_time ASC) AS fcw,
           FIRST_VALUE(recipient_id) OVER(PARTITION BY caller_id, DATE(call_time) ORDER BY call_time DESC) AS lcw
    FROM cte
)

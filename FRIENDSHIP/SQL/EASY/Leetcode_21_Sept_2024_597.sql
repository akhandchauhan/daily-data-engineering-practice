...
-- in SQL).This table contains the ID of the user who sent the request, the ID of the user
...
-- Find the overall acceptance rate of requests, which is the number of acceptance 
...
WITH accept_info AS (
    SELECT COUNT(*) AS unique_accepted_requests
    FROM (
        SELECT COUNT(*) AS cnt
        FROM RequestAccepted
...
SELECT 
        COALESCE(
            ROUND(
                     unique_accepted_requests/request_count
                     ,2)
            ,0.00) AS accept_rate
FROM request_info 
CROSS JOIN accept_info;
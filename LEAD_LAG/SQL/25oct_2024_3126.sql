...
-- | session_status | enum     |
-- +----------------+----------+
-- (server_id, status_time, session_status) is the primary key (combination of columns with
...
-- Write a solution to find the total time when servers were running. The output should be
-- rounded down to the nearest number of full days.
...
INSERT INTO Servers (server_id, status_time, session_status)
VALUES
...
SELECT FLOOR(SUM(TIMESTAMPDIFF(SECOND, status_time, next_status_time))/(3600 * 24)) AS total_uptime_days
FROM server_run_info
WHERE session_status ='start';


-------------------------------------------------------------------------------------------------------
-- m3 row_number and join
...
WITH server_info as (
    SELECT 
        ...
        status_time,
        session_status,
        ROW_NUMBER() OVER(PARTITION BY server_id ORDER BY status_time) AS time_rank
    FROM Servers 
)
SELECT 
    FLOOR(SUM(TIMESTAMPDIFF(SECOND, s1.status_time, s2.status_time))/(3600 * 24)) AS total_uptime_days
FROM server_info s1 
JOIN server_info s2 
ON s1.server_id = s2.server_id
AND s1.time_rank = s2.time_rank - 1

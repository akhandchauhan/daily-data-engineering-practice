...
df = df.sort_values(by = ['flight_id','booking_time'] , ascending =[True,True] )
df['rnk'] = df.groupby('flight_id').cumcount() + 1
df['Status'] = (df['capacity'] >= df['rnk']).astype(int).apply(lambda x: 'Confirmed' if x ==1 else 'Waitlisted')
df = df[['passenger_id','Status']].sort_values(by = 'passenger_id', ascending = True)
print(df)
